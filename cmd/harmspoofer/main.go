...
func main() {
	...
	app.Action = func(c *cli.Context) error {
		...
		if err != nil {
			logger.Fatal().Err(err).Msg("pcap.OpenLive error")
		}
		defer handle.Close()
		...
		if strings.Contains(serverAddrStr, "/") {
			filter = "net " + serverAddrStr
		} else {
			filter = "host " + serverAddrStr
		}

		filter = "inbound and (tcp[tcpflags] & tcp-fin) == 0 and (tcp[tcpflags] & tcp-rst) == 0 and " + filter

		err = handle.SetBPFFilter(filter)
		if err != nil {
			logger.Fatal().Err(err).Msg("set bpf filter error")
		}
	...
	err := app.Run(os.Args)
	if err != nil {
		logger.Fatal().Err(err).Msg("app.Run error")
	}
}
...
func sendthread(interfaceName string, packetQueue chan []byte) {
	...
	handle, err := pcap.OpenLive(interfaceName, MaxPacketSize, true, pcap.BlockForever)
	if err != nil {
		logger.Fatal().Err(err).Msg("open send interface error")
	}
	defer handle.Close()
	...
	if tcpLay != nil {
		...
		if tcp.SYN && !tcp.ACK {
			...
			if err := gopacket.SerializeLayers(buffer, gopacket.SerializeOptions{FixLengths: true, ComputeChecksums: true},
				...
				fmt.Printf("Error serializing packet: %c", err)
				return
			}
...
func GoDotEnvVariable(key string) string {
	...
	if err != nil {
		logger.Fatal().Err(err).Msg("error opening .env")
	}
